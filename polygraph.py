...
class PolygraphRNN(object):
    ...
    Args:
        num_units (int):           Number of hidden units on a RNN layer
        num_layers (int):          Number of RNN layers
        rnn_type (str, optional):  Type of RNN, e.g., 'lstm', 'gru' or 'rnn'.
                                   Defaults to 'lstm'
        direction (str, optional): Choice of uni- or bi-directional RNN
                                   e.g., 'uni' or 'bi'. Defaults to 'uni'
        lr (float, optional):      Learning rate. Defaults to 1e-3

    Attributes:
    Private
        _num_units (int):  Number of hidden units on a RNN layer
        _num_layers (int): Number of RNN layers
        _rnn_type (str):   Type of RNN
        _direction (str):  Choice of uni- or bi-directional RNN

    Tensors
        inputs:            A `Tensor` holds inputs
        targets:           A `Tensor` holds targets
        seq_lens:          A `Tensor` holds raw sequence lengths (steps)
        keep_prob:         A `Tensor` holds dropout probability
        pos_weight:        A `Tensor` holds positive-class weight
        logits:            A `Tensor` holds logit values
        loss:              A `Tensor` holds loss value
        predictions:       A `Tensor` holds predictions
        lr:                A `Tensor` holds loss value
        optimizer:         A `Tensor` holds optimizer
        accuracy:          A `Tensor` holds accuracy value
    ...
    def _add_placeholders(self):
        ...
        self.pos_weight = tf.placeholder(tf.float32, name='pos_weight')
    ...
    def _add_rnn(self, inputs, seq_lens):
        ...
        def rnn_cell(num_units, keep_prob, rnn_type='lstm'):
            if rnn_type == 'lstm':
                ...
                raise ValueError("Unknown rnn_type as '{}'".format(rnn_type))
            return tf.nn.rnn_cell.DropoutWrapper(cell,
                                                 output_keep_prob=keep_prob)

        # For sequence classification, we need the very last relevant output
        # that supposes to contain the entire information of the sequence
        # from the history
        if self._direction == 'uni':
            ...
            # Extract the last relevant output from dynamic run using
            # variable-length sequences
            last = self._extract_relevant(outputs, seq_lens - 1)

        elif self._direction == 'bi':
            # Bidirectional
            cell_fw = rnn_cell(self._num_units, self.keep_prob, self._rnn_type)
            cell_bw = rnn_cell(self._num_units, self.keep_prob, self._rnn_type)
            ...
            # last_fw = self._extract_relevant(output_fw, seq_lens - 1)
            batch_range = tf.range(tf.shape(output_fw)[0])
            indices = tf.stack([batch_range, seq_lens - 1], axis=1)
            last_fw = tf.gather_nd(output_fw, indices)

            # The last output from the backward layer locates at the first step
            last_bw = output_bw[:, 0, :]
            last = tf.concat([last_fw, last_bw], 1)

        else:
            raise ValueError("Unknown direction as '{}'".format(
                self._direction))
    ...
    def _add_prediction_op(self):
        ...
        last = self._add_rnn(self.inputs, self.seq_lens)
        ...
        # For binary classification, just need one unit from the fully-connected
    ...
    def _add_metric_op(self):
        ...
        correct = tf.equal(tf.argmax(self.targets, 1),
    ...
    def run(self, session,
        ...
        """
        feed = self._add_feed_dict(X_batch,
                                   y_batch,
                                   l_batch,
                                   keep_prob,
                                   pos_weight)
        ...
        loss, accuracy, pred = session.run([self.loss,
                                            self.accuracy,
                                            self.predictions], feed)
    ...
    def _extract_relevant(outputs, indices):
        """
        Extract the relevant output when using dynamic_rnn with variable-length
        sequence inputs
...
class PolygraphCRNN(PolygraphRNN):
    ...
    def __init__(self, num_units,
        ...
        super(PolygraphCRNN, self).__init__(
            num_units,
            num_layers,
            rnn_type=rnn_type,
            direction=direction,
    ...
    @staticmethod
    def _add_cnn(inputs_cnn, seq_lens):
        # CNN 3 layers of Conv2D + MaxPooling
        max_step = tf.shape(inputs_cnn)[1]

        inputs_cnn = tf.reshape(inputs_cnn,
                                [-1, max_step, Config.num_feats, 1])
        ...
        # Sequence length reduced to 1/8
        reduced_lens = tf.cast(tf.divide(seq_lens, 8), tf.int32)

        return outputs_cnn, reduced_lens
    ...
    def _add_prediction_op(self):
        ...
        """
        # CNN outputs (4D-Tensor)
        outputs_cnn, reduced_lens = self._add_cnn(self.inputs, self.seq_lens)

        # Reshape Tensor for RNN (3D-Tensor)
        batch_size = tf.shape(outputs_cnn)[0]
        # Max steps has been reduced to 1/8 due to 3 levels of pooling
        reduced_steps = tf.shape(outputs_cnn)[1]
        # 'inputs_rnn' is of shape (batch_size, reduced_steps, num_filters=64)
        inputs_rnn = tf.reshape(outputs_cnn, [batch_size, reduced_steps, 64])

        # RNN last output
        last = self._add_rnn(inputs_rnn, reduced_lens)
        ...
        # For binary classification, we need one unit from the fully-connected
        # Weights are initialized using truncated normal initializer
        # Biases are initialized with non-zero for layer pre-activation
        self.logits = tf.layers.dense(
            drop,
...
def run_experiment(model_, data, config, log_file):
    """
    Run experiment with the given model, data and setup
    ...
    header = "dir: {}, num_units: {}, num_layers: {}, keep_prob: {}, lr: {}\n"
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            ...
            for epoch in range(1, config.num_epochs + 1):
                ...
                X_batches, y_batches, l_batches = make_batches(
                    X_train,
                    y_train,
                    l_train,
                    batch_size=config.batch_size)
                print("{}/{}:".format(epoch, config.num_epochs))
                for i, (X_batch, y_batch, l_batch) in enumerate(zip(X_batches,
                                                                    y_batches,
                                                                    l_batches),
                                                                start=1):
                    ...
                    # pos_weight = num_F/num_T, used on the positive class
                    num_neg_batch, num_pos_batch = class_batch
                    batch_loss, batch_acc, _ = model.run(
                        session,
                ...
                # Evaluation
                test_loss, test_acc, y_pred = model.run(
                    session,
                    X_test,
                    y_test,
                    l_test,
